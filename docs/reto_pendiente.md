...
- [ ] **Arquitectura y Limpieza:** Crear las clases `MarcoContrato` y `LaminaContrato` en el paquete `pildorasinformatica_gemini`. Seguir el patrón de diseño donde el `JFrame` hospeda al `JPanel`.
- [ ] **Integración en Sistema:** Añadir un botón "Gestión de Contratos" en el `MarcoCentradoComando` para abrir este módulo (`DISPOSE_ON_CLOSE`).
...
- [ ] **Grupos de Selección Excluyente (JRadioButton):**
...
- [ ] **Estándares Profesionales:** Validar uso de `private`, anotaciones `@Override` y nomenclaturas correctas.
